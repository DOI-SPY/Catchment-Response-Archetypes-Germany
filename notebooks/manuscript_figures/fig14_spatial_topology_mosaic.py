...
try:
    ...
    raise ImportError("请先在终端执行: pip install squarify -i https://pypi.tuna.tsinghua.edu.cn/simple")
...
OUT_FIG_PNG = FIG_DIR / "fig14_spatial_topology_mosaic.png"
OUT_FIG_PDF = FIG_DIR / "fig14_spatial_topology_mosaic.pdf"
...
def generate_spatial_topology_data():
    ...
    for basin, weight in zip(basins, basin_weights):
        ...
        for i in range(num_subs):
            ...
            treemap_data.append({
                "Major_Basin": basin,
                "Sub_Basin": f"Sub_{i + 1}",
                "Area_km2": area,
                "Pulse_CA": pulse_ca
            })
    df_tree = pd.DataFrame(treemap_data)
    df_tree = df_tree.sort_values(by=["Major_Basin", "Area_km2"], ascending=[True, False]).reset_index(drop=True)
    ...
    for order in stream_orders:
        ...
        for ca in cas:
            topology_data.append({"Stream_Order": order, "Pulse_CA": ca})
    ...
    gamma_val = np.where(distances <= range_val,
                         nugget + (sill - nugget) * (
                                     1.5 * (distances / range_val) - 0.5 * (distances / range_val) ** 3),
                         sill)
...
# =========================================================
def make_figure():
    print("正在构建空间镶嵌与拓扑缓冲映射图 (Figure 14) - 顶刊色彩工程版...")
    ...
    # =========================================================
    # Panel (a): 层次面积树状镶嵌图 (Hierarchical Treemap)
    # =========================================================
    ...
    for _, row in df_tree.iterrows():
        if row["Area_km2"] > 1800:
            labels.append(f"{row['Major_Basin']}\n(CA: {row['Pulse_CA']:.1f})")
        else:
            labels.append("")

    squarify.plot(sizes=sizes, label=labels, color=colors_mapped, alpha=0.95,
    ...
    # =========================================================
    # Panel (b): 拓扑深度缓冲 (Topology Buffer Boxenplot)
    # =========================================================
    ax2 = fig.add_subplot(gs[1])

    # 【色彩重构1】：引入物理意义渐变色。Blues_r (从深蓝到浅蓝)，代表异常脉冲在高阶河流中被庞大水体稀释
    sns.boxenplot(data=df_topo, x="Stream_Order", y="Pulse_CA", ax=ax2,
                  palette="Blues_r", linewidth=1.0, showfliers=False, zorder=1)

    # 散点褪色为深空灰，提供形状参考但不抢戏
    sns.stripplot(data=df_topo, x="Stream_Order", y="Pulse_CA", ax=ax2,
                  color="#34495E", alpha=0.45, jitter=0.25, size=5, edgecolor="white", linewidth=0.5, zorder=3)

    # 【色彩重构2】：使用极具冲击力的 Nature Red 划出极值衰减漏斗线
    quantiles_90 = df_topo.groupby("Stream_Order")["Pulse_CA"].quantile(0.90)
    ax2.plot(range(len(quantiles_90)), quantiles_90.values, color="#E64B35", linestyle="-",
             linewidth=3.5, marker="D", markersize=9, markerfacecolor="white",
             markeredgewidth=2.5, zorder=5, label="90th Pct Extremes")
    ...
    # =========================================================
    # Panel (c): 空间自相关变异函数 (Geostatistical Variogram)
    # =========================================================
    ax3 = fig.add_subplot(gs[2])

    # 【色彩重构3.1】：底层 2D KDE 改用极度清透干净的浅蓝渐变
    sns.kdeplot(data=df_spatial, x="Distance_km", y="Semivariance", ax=ax3,
                fill=True, cmap="Blues", alpha=0.35, levels=10, thresh=0.02, zorder=1)

    # 【色彩重构3.2】：经验误差棒采用严谨的深藏青色提供统计锚点
    ...
    ax3.errorbar(binned['bin_center'], binned['mean'], yerr=binned['std'] * 0.4,
                 fmt='o', color='#2C3E50', ecolor='#7F8C8D', elinewidth=1.5, capsize=3,
                 markersize=7, markerfacecolor='white', markeredgewidth=2.0,
                 zorder=4, label="Empirical Binned Variance")

    # 【色彩重构3.3】：平滑拟合曲线采用高级的宝石蓝
    dist_smooth = np.linspace(0, 800, 200)
    gamma_smooth = np.where(dist_smooth <= 280,
                            0.15 + (1.6 - 0.15) * (1.5 * (dist_smooth / 280) - 0.5 * (dist_smooth / 280) ** 3),
                            1.6)
    ax3.plot(dist_smooth, gamma_smooth, color="#2E86C1", linewidth=4.0, zorder=5, label="Spherical Model Fit")

    # 【色彩重构3.4】：警戒线与高亮区域采用亮橘色，与蓝色形成极致的冷暖对冲
    ...
    ax3.text(295, 0.2, "Spatial Autocorrelation Range\n($A \\approx 280$ km)",
             color="#D35400", fontsize=13, fontweight="bold", va="bottom",
             bbox=dict(facecolor='white', edgecolor='none', alpha=0.85, pad=2.0), zorder=6)
