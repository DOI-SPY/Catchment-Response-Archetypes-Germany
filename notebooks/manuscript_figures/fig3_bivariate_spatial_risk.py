...
# 【修改点 1】：更新输出文件名为 fig3，契合全局逻辑
OUT_FIG_PNG = FIG_DIR / "fig3_bivariate_spatial_risk.png"
OUT_FIG_PDF = FIG_DIR / "fig3_bivariate_spatial_risk.pdf"
...
# =========================================================
def print_manuscript_report(df: pd.DataFrame):
    """自动生成用于正文撰写的精准统计数据"""
    total_catchments = len(df)

    # 提取高危流域 (农业最高 1/3 且 CMD 也是最高 1/3)
    high_risk_df = df[(df["CMD_Class"] == 2) & (df["Agri_Class"] == 2)]
    high_risk_count = len(high_risk_df)
    high_risk_pct = (high_risk_count / total_catchments) * 100

    # 获取高危阈值的物理边界
    agri_threshold = df[df["Agri_Class"] == 2]["Agri_Frac"].min() * 100  # 转为百分比
    cmd_threshold = df[df["CMD_Class"] == 2]["CMD_3m"].min()

    print("\n" + "=" * 70)
    print(" 📄 [Text-Data Integration] 专属正文数据填空报告 (New Fig 3) ")
    print("=" * 70)
    print("请将以下真实数据填入本文 3.1 节的对应 [括号] 内：\n")
    print(f"[DATA_TOTAL_CATCHMENTS] = {total_catchments} (分析的总流域数量)")
    print(f"[DATA_HIGH_RISK_PCT]    = {high_risk_pct:.1f}% (双重高危流域占比, 如15.5%)")
    print(f"[DATA_HIGH_RISK_COUNT]  = {high_risk_count} (双重高危流域的绝对数量)")
    print(f"[DATA_AGRI_THRESHOLD]   = {agri_threshold:.1f}% (划分为高农业的下限阈值)")
    print(f"[DATA_CMD_THRESHOLD]    = {cmd_threshold:.1f} (划分为深重干旱记忆的下限阈值)")
    print("=" * 70 + "\n")
...
def draw_bivariate_legend(ax):
    ...
    for i in range(3):
        for j in range(3):
            rect = patches.Rectangle((i * sz, j * sz), sz, sz, linewidth=1.5, edgecolor='white',
                                     facecolor=BIVARIATE_COLORS[(i, j)], zorder=1)
            ax.add_patch(rect)
...
def plot_waffle_chart(ax, df):
    ...
    for y in range(10):
        for x in range(10):
            rect = patches.Rectangle((x, 9 - y), 1.0, 1.0, facecolor=colors_list[idx], edgecolor="white", linewidth=2.0)
            ax.add_patch(rect)
            idx += 1
    ax.set_xlim(0, 10);
    ax.set_ylim(0, 10)
...
def make_figure():
    print("正在生成新版 Figure 3 (空间基线与数据抽取)...")
    plt.rcParams["font.family"] = "Times New Roman"

    biv_df = build_bivariate_data()
    print_manuscript_report(biv_df)  # 💡 调用报告生成器
    ...
    if catch_shp is not None: catch_shp.plot(ax=ax_map, facecolor="#f8f9fa", edgecolor="#b0b0b0", linewidth=0.6,
                                             zorder=1)
    if st_shp is not None: st_shp.plot(ax=ax_map, color=st_shp["Color"], markersize=65, edgecolor="white",
